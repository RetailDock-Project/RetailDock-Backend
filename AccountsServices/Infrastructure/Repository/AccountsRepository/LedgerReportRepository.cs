...
";
        ...
        public async Task<LedgerClosinBalanceDTO> GetLedgerClosingBalanceAsync(  Guid organizationId, Guid ledgerId, DateTime? startDate, DateTime? endDate)
        {
            var start = startDate ?? new DateTime(DateTime.UtcNow.Year, DateTime.UtcNow.Month, 1);
            var end = endDate ?? DateTime.UtcNow;
        ...
        WHERE T.LedgerId = @LedgerId
          ...
          AND V.VoucherDate BETWEEN @StartDate AND @EndDate;";
            ...
            // Return DTO
            return new LedgerClosinBalanceDTO
            {
                ClosingBalance = closingBalance,
                ClosingType = closingType
            };
